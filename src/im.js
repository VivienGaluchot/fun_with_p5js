...
class AddBox extends Box {
  constructor() {
    super("add",
      // ins
      [new In("a", base_types.number), new In("b", base_types.number)],
      // out
      new Out("a+b", base_types.number),
      // compute
      function(ins) {
...
var cBox4 = new CBox(base_types.number, 4);
console.assert(cBox4.is_valid(true), "simple box invalid", cBox4);
as.register_box(cBox4);

var cBox7 = new CBox(base_types.number, 7);
console.assert(cBox7.is_valid(true), "simple box invalid", cBox7);
as.register_box(cBox7);
console.assert(as.is_valid(true), "Assembler invalid", as);
...
as.register_box(addBox);
console.assert(addBox.is_valid(true), "simple box invalid", addBox);
console.assert(as.is_valid(true), "Assembler invalid", as);
